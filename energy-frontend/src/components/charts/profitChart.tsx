...
function ProfitChart(props: { className?: string }) {
  ...
  /*useEffect(() => {
    if (data) {
      ...
      const spotPrices = data.spot_prices.map((price) => price.amount);
      const sellingPrices = data.selling_prices.map((price) => price.amount);
      const dates = data.selling_prices.map((price) => price.date);
