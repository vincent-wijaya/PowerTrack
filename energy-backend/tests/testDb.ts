...
export const connectToTestDb = async () => {
  ...
  await rootSequelize.query(`CREATE DATABASE ${dbName};`).catch(
    (err: any) => {
      console.log(`Error creating database ${dbName}: ${err.message}`);
    }
  );
  await rootSequelize.close();
  let testSequelize = new Sequelize(`${process.env.TEST_DATABASE_CLUSTER}${dbName}`);
...
export const dropTestDb = async (sequelize: Sequelize) => {
  ...
  await rootSequelize.query(`DROP DATABASE ${dbName};`).catch(
    (err: any) => {
