...
router.get('/generator', async (req, res) => {
  try {
    ...
    // Return error 400 if su/burb_id (if provided) does not exist
    if (suburb_id && result.length === 0) {
      return res.status(400).send('No data found for the provided parameters.');
    }
