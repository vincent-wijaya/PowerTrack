 ...
 */
export const generatePeriodicReports = async (sequelize: Sequelize, startTime: string, endTime: string) => {
  // Define models
  ...
  await Report.bulkCreate(consumers.map((consumer, index) => ({
  ...
  console.log(`Generated periodic reports for the period ${startTime} to ${endTime} for ${consumers.length} consumers`);

  // Send emails to consumers
  const consumersToEmail = consumers.filter(consumer => consumer.email_address);
  ...
  consumersToEmail.forEach(async (consumer) => {
    ...
    const subject = "Energy report for the period " + startTime + " to " + endTime;
